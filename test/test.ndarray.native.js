...
var sapx = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'the function adds a constant to each strided array element', opts, function test( t ) {
	var expected;
	var x;
...
tape( 'if `alpha` equals `0`, the function returns the input array unchanged', opts, function test( t ) {
	var expected;
	var x;
...
tape( 'if `stride` is equal to `1`, the function efficiently adds a constant to each element in a strided array', opts, function test( t ) {
	var expected;
	var alpha;
	var x;
	var i;
